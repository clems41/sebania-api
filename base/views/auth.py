...
class AuthViewSet(ViewSet):
    ...
    @action(detail=False, methods=['put'], url_path='change-password', serializer_class=ChangePasswordSerializer,
            url_name="change-password", basename="auth-change-password")
    ...
    @extend_schema(responses=None,
                   ...
                   parameters=[OpenApiParameter(name="email", required=True,
    ...
    @action(detail=False, methods=['get'], url_path='me', serializer_class=UserSerializer, url_name="me",
            basename="auth-me")
    def me(self, request):
        return Response(UserSerializer(request.user).data, status=status.HTTP_200_OK)

    @extend_schema(description="Retourne un booléen pour savoir si l'adresse email existe déjà",
                   parameters=[
